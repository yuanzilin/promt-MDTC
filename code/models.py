...
class MlpFeatureExtractor(nn.Module):
    def __init__(self,
        ...
        for i in range(num_layers):
            ...
            if i == 0:
                # print("models,150,input_size={},hidden_size[0]={}".format(input_size,hidden_sizes[0]))
                self.net.add_module('f-linear-{}'.format(i), nn.Linear(input_size, hidden_sizes[0]))
            else:
                # print("models,150,hidden_sizes[i-1]={},hidden_size[i]={}".format(hidden_sizes[i-1],hidden_sizes[i]))
    ...
    def forward(self, input):
        # print("model,167,input=",input)
        # print("model,168,input.shape=",input.shape)
        network=self.net(input)
        # print("model,169")
